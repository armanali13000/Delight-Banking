...
function DetailTable({ title, rows, columns, empty = "No records." }) {
  return <section className="admin-card admin-table-wrap"><h2>{title}</h2>{rows?.length ? <table className="admin-table"><thead><tr>{columns.map((column) => <th key={`${column.key}-${column.label}`}>{column.label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.id || row.uid || row.orderId || row.transactionId}>{columns.map((column) => <td key={`${column.key}-${column.label}`}>{column.render ? column.render(row) : row[column.key] || "-"}</td>)}</tr>)}</tbody></table> : <AdminEmptyState title={title} text={empty} />}</section>;
}
...
function AdminTransactionsPage({ admin }) {
  ...
  return <PermissionGate admin={admin} permission="payments.view"><AdminPageHeader eyebrow="Transactions" title="Transactions" description="Safe Cashfree payment records, webhook state, activation state, and reconciliation tools." admin={admin} /><AdminOperationalFilters filters={filters} setFilters={setFilters} type="transactions" onExport={exportCsv} canExport={adminHasPermission(admin, "reports.export")} />{message ? <AdminEmptyState title="Transactions" text={message} /> : data?.items?.length ? <><DetailTable title="Transactions" rows={data.items} columns={[{ key: "internalTransactionId", label: "Transaction" }, { key: "cashfreePaymentId", label: "Cashfree payment" }, { key: "cashfreeOrderId", label: "Cashfree order" }, { key: "userEmail", label: "User" }, { key: "amount", label: "Amount", render: (row) => formatPrice(row.amount) }, { key: "currency", label: "Currency" }, { key: "paymentMethod", label: "Method" }, { key: "normalizedStatus", label: "Status" }, { key: "webhookVerified", label: "Webhook", render: (row) => row.webhookVerified ? "Verified" : "Pending" }, { key: "subscriptionActivated", label: "Access", render: (row) => row.subscriptionActivated ? "Activated" : "Not activated" }, { key: "id", label: "Action", render: (row) => <a className="text-button" href={`${appBase}admin/transactions/${encodeURIComponent(row.id)}`}>Open</a> }]} /><AdminPager {...data} onPage={(page) => setFilters({ ...filters, page })} /></> : <AdminEmptyState title="No transactions found" text="No transactions match the selected filters." />}</PermissionGate>;
}
...
function AdminTransactionDetailPage({ admin, id }) {
  ...
  return <PermissionGate admin={admin} permission="payments.view"><AdminPageHeader eyebrow="Transaction" title={transaction?.internalTransactionId || "Transaction details"} description="Safe payment identifiers only. Full gateway payloads and secrets are never shown." admin={admin} />{message ? <AdminEmptyState title="Transaction details" text={message} /> : transaction && <><section className="admin-profile-card"><dl className="student-details admin-security-fields"><div><dt>Transaction ID</dt><dd>{transaction.internalTransactionId}</dd></div><div><dt>Cashfree payment</dt><dd>{transaction.cashfreePaymentId || "None"}</dd></div><div><dt>Cashfree order</dt><dd>{transaction.cashfreeOrderId || "None"}</dd></div><div><dt>User</dt><dd>{transaction.userEmail || "Unknown"}</dd></div><div><dt>Amount</dt><dd>{formatPrice(transaction.amount)} {transaction.currency}</dd></div><div><dt>Status</dt><dd>{transaction.normalizedStatus}</dd></div><div><dt>Webhook</dt><dd>{transaction.webhookVerified ? "Verified" : "Pending"}</dd></div><div><dt>Subscription activation</dt><dd>{transaction.subscriptionActivated ? "Activated" : "Not activated"}</dd></div><div><dt>Captured</dt><dd>{formatDate(transaction.capturedAt)}</dd></div><div><dt>Last reconciliation</dt><dd>{formatDate(transaction.lastReconciledAt)}</dd></div></dl>{adminHasPermission(admin, "payments.reconcile") && <button className="primary-button" type="button" onClick={reconcile}>Reconcile with Cashfree</button>}</section><DetailTable title="Safe event timeline" rows={detail.timeline || []} columns={[{ key: "label", label: "Event" }, { key: "at", label: "Date", render: (row) => formatDate(row.at) }]} /></>}</PermissionGate>;
}
