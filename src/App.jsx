...
function AdminUsersPage({ admin }) {
  ...
  return <PermissionGate admin={admin} permission="users.view"><AdminPageHeader eyebrow="Users" title="Registered users" description="Firebase Authentication users merged with safe Firestore profile and subscription state." admin={admin} /><AdminOperationalFilters filters={filters} setFilters={setFilters} type="users" onExport={exportCsv} canExport={adminHasPermission(admin, "reports.export")} />{message ? <AdminEmptyState title="Unable to load users" text={message} onRetry={() => setFilters({ ...filters })} /> : data?.items?.length ? <><section className="admin-card admin-table-wrap"><table className="admin-table"><thead><tr><th>User</th><th>Email</th><th>Phone</th><th>Provider</th><th>Verified</th><th>Profile</th><th>Status</th><th>Registered (IST)</th><th>Last sign-in</th><th>Active plans</th><th>Current plan</th><th>Expiry</th><th>Action</th></tr></thead><tbody>{data.items.map((user) => <tr key={user.uid}><td>{user.photoURL ? <img className="admin-inline-avatar" src={user.photoURL} alt="" /> : <span className="admin-inline-avatar">{(user.displayName || "U").slice(0, 1)}</span>}{user.displayName}</td><td>{user.email}</td><td>{user.phone || "-"}</td><td>{user.provider}</td><td>{user.emailVerified ? "Verified" : "Unverified"}</td><td>{titleLabel(user.profileStatus)}</td><td><span className={`admin-status-badge ${user.accountStatus}`}>{user.accountStatus}</span></td><td>{formatDate(user.createdAt)}</td><td>{formatDate(user.lastSignInAt)}</td><td>{user.activeSubscriptionCount}</td><td>{user.currentPlan}</td><td>{formatDate(user.subscriptionExpiry)}</td><td><a className="text-button" href={`${appBase}admin/users/${encodeURIComponent(user.uid)}`}>Open Profile</a></td></tr>)}</tbody></table><div className="admin-mobile-list">{data.items.map((user) => <article className="admin-mobile-card" key={user.uid}><header><div><strong>{user.displayName || user.email || "User"}</strong><span>{user.email || "No email"}</span></div><span className={`admin-status-badge ${user.accountStatus}`}>{user.accountStatus}</span></header><div className="admin-mobile-field"><span>Phone</span><strong>{user.phone || "-"}</strong></div><div className="admin-mobile-field"><span>Provider</span><strong>{user.provider || "-"}</strong></div><div className="admin-mobile-field"><span>Verified</span><strong>{user.emailVerified ? "Verified" : "Unverified"}</strong></div><div className="admin-mobile-field"><span>Profile</span><strong>{titleLabel(user.profileStatus)}</strong></div><div className="admin-mobile-field"><span>Registered (IST)</span><strong>{formatDate(user.createdAt)}</strong></div><div className="admin-mobile-field"><span>Last sign-in</span><strong>{formatDate(user.lastSignInAt)}</strong></div><div className="admin-mobile-field"><span>Active plans</span><strong>{user.activeSubscriptionCount}</strong></div><div className="admin-mobile-field"><span>Current plan</span><strong>{user.currentPlan || "No active plan"}</strong></div><div className="admin-mobile-field"><span>Expiry</span><strong>{formatDate(user.subscriptionExpiry)}</strong></div><div className="admin-row-actions"><a className="ghost-button" href={`${appBase}admin/users/${encodeURIComponent(user.uid)}`}>Open Profile</a></div></article>)}</div></section><AdminPager {...data} onPage={(page) => setFilters({ ...filters, page })} /></> : <AdminEmptyState title="No users found" text="No registered users match the selected filters." />}</PermissionGate>;
}
