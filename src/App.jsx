...
import { formatRegistrationDate } from "./utils/adminDate.js";
...
function transactionDateDisplay(transaction) {
  if (!transaction?.transactionDateAt) return "Transaction date unavailable";
  return transaction.transactionDateLabel + ": " + transaction.transactionDateIst;
}
...
function CheckoutPage({ variantId }) {
  ...
  const [checkoutProfileReady, setCheckoutProfileReady] = useState(previewMode);
  const [accepted, setAccepted] = useState({ terms: false, refund: false, privacy: false });
  const [status, setStatus] = useState("idle");
  const [message, setMessage] = useState("");
  useEffect(() => listenToAuth((nextUser) => { setUser(nextUser); if(nextUser){getStudentProfile().then(result=>{setProfile(result.profile||{});setCheckoutProfileReady(Boolean(result.profile?.profileComplete))}).catch(()=>setCheckoutProfileReady(false))}else{setProfile({});setCheckoutProfileReady(previewMode);if(!previewMode)setAuthMode("signin")} }), [previewMode]);
  ...
  const canPay = !previewMode && user && checkoutProfileReady && accepted.terms && accepted.refund && accepted.privacy && status !== "processing";
  async function pay() {
    ...
    try {
      const order = await createPaymentOrder(variant.variantId, { name: profile.fullName || profile.name, phone: profile.mobile || profile.phone, address: profile.address });
...
function StudentContentDeskPage({ path }) {
  ...
  const view = resourceMatch ? "resource" : targetMatch ? "target" : path.includes("/resources") ? "resources" : path.includes("/classes") ? "classes" : path.includes("/notification-settings") ? "notification-settings" : path.includes("/notifications") ? "notifications" : path.includes("/profile") ? "profile" : path.includes("/targets") ? "targets" : "dashboard";
  useEffect(() => listenToAuth((nextUser) => { setUser(nextUser); setAuthReady(true); }), []);
  useEffect(() => { if(user)getStudentProfile().then(result=>setProfileSummary(result.profile)).catch(()=>setProfileSummary(null)); }, [user?.uid]);
  async function loadContent() {
    setLoadState("loading");
    setMessage("Loading student content...");
    try {
      const result = resourceMatch ? await getStudentResource(decodeURIComponent(resourceMatch[1])) : targetMatch ? await getStudentTarget(decodeURIComponent(targetMatch[1])) : view === "resources" ? await getStudentResources() : view === "classes" ? await getStudentClasses() : view === "targets" ? await getStudentTargets() : view === "profile" ? await getStudentProfile() : await getStudentContentDashboard();
      setData(result);
      setMessage("");
  ...
  if (!user) return <Shell user={user} onAuth={setAuthMode}><main className="desk-page"><section className="admin-gate"><div className="premium-card gate-card student-login-gate"><Brand small="Student Desk" /><h1>Student Desk Login</h1><p>Sign in to view protected resources, classes, targets, subscriptions, and payment history.</p><button className="primary-button" type="button" onClick={() => setAuthMode("signin")}>Login</button></div></section></main>{authMode && <AuthModal mode={authMode} onClose={() => setAuthMode(null)} onUser={setUser} />}</Shell>;
...
function StudentNotificationsPage(){
 ...
 return <section className="notification-page"><header><div><h2>Notifications</h2><span>{state.data?.unreadCount||0} unread · {state.data?.total||0} total</span></div><div className="form-actions"><a className="ghost-button" href={appBase+"student-desk/notification-settings"}>Settings</a><button className="ghost-button" onClick={async()=>{await markAllStudentNotifications();load()}}>Mark all read</button><button className="ghost-button" onClick={load}>Refresh</button></div></header><nav className="notification-filters">{["all","unread","payments","subscriptions","targets","resources","classes","announcements"].map(x=><button className={filter===x?"active":""} onClick={()=>{setFilter(x);setPage(1)}} key={x}>{titleLabel(x)}</button>)}</nav>{state.loading?<AdminLoadingSkeleton/>:state.error?<AdminEmptyState title="Unable to load notifications" text={state.error} onRetry={load}/>:items.length?<div className="notification-records">{items.map(n=><article className={n.readAt?"":"unread"} key={n.id}><strong>{n.title}</strong><p>{n.message}</p><div className="meta-row"><span>{titleLabel(n.eventType)}</span><span>{formatDate(n.createdAt)}</span><span>{n.related?.planId||""}</span></div><button onClick={async()=>{await markStudentNotification(n.id);load()}}>Mark read</button><button onClick={async()=>{await markStudentNotification(n.id);routeTo(n.destination.startsWith("/")&&!n.destination.startsWith("//")?n.destination:"/student-desk")}}>Open</button></article>)}</div>:<AdminEmptyState title="No notifications" text="No notifications match this filter."/>}<div className="admin-pagination"><button disabled={page===1} onClick={()=>setPage(page-1)}>Previous</button><span>Page {page}</span><button disabled={page*size>=all.length} onClick={()=>setPage(page+1)}>Next</button></div></section>
}
function StudentProfilePage({ initial, onSaved }) {
 const [profile,setProfile]=useState(initial||{}),[status,setStatus]=useState(""),[errors,setErrors]=useState({}),[saving,setSaving]=useState(false);
 useEffect(()=>{if(initial)setProfile(initial)},[initial]);
 function update(field,value){setProfile(current=>({...current,[field]:value}));setErrors(current=>({...current,[field]:""}))}
 async function submit(event){event.preventDefault();const next={};if(!String(profile.fullName||"").trim())next.fullName="Full name is required.";if(!String(profile.mobile||"").trim())next.mobile="Mobile number is required.";setErrors(next);if(Object.keys(next).length)return;setSaving(true);setStatus("Saving profile...");try{const result=await saveStudentProfile(profile);setProfile(result.profile);onSaved?.(result.profile);setStatus("Profile saved successfully.")}catch(error){setStatus(error.message)}finally{setSaving(false)}}
 return <form className="profile-edit-panel student-profile-form" onSubmit={submit}><header><div><p className="eyebrow">Profile</p><h2>{profile.profileComplete?"Your profile":"Complete your profile"}</h2><p>Keep these private details current for account support and mentoring.</p></div>{profile.profileComplete&&<span className="status-pill">Complete</span>}</header><div className="profile-form-grid"><label>Full name<input value={profile.fullName||""} onChange={e=>update("fullName",e.target.value)} autoComplete="name" required />{errors.fullName&&<small className="field-error">{errors.fullName}</small>}</label><div className="phone-field"><label>Country code<select value={profile.countryCode||"+91"} onChange={e=>update("countryCode",e.target.value)}><option value="+91">India +91</option><option value="+1">USA/Canada +1</option><option value="+44">UK +44</option><option value="+61">Australia +61</option><option value="+971">UAE +971</option></select></label><label>Mobile number<input value={profile.mobile||""} onChange={e=>update("mobile",e.target.value)} inputMode="tel" autoComplete="tel" required />{errors.mobile&&<small className="field-error">{errors.mobile}</small>}</label></div><label>Date of birth<input type="date" value={profile.dateOfBirth||""} onChange={e=>update("dateOfBirth",e.target.value)} /></label><label>City<input value={profile.city||""} onChange={e=>update("city",e.target.value)} /></label><label>State<input value={profile.state||""} onChange={e=>update("state",e.target.value)} /></label><label>Country<input value={profile.country||"India"} onChange={e=>update("country",e.target.value)} /></label><label>Postal / PIN code<input value={profile.postalCode||""} onChange={e=>update("postalCode",e.target.value)} inputMode="numeric" /></label><label>Profile photograph URL<input type="url" value={profile.photoURL||""} onChange={e=>update("photoURL",e.target.value)} placeholder="https://..." /></label><label className="wide-field">Address / location<textarea rows="3" value={profile.address||""} onChange={e=>update("address",e.target.value)} /></label><label className="wide-field">Exam interests<input value={(profile.examInterests||[]).join(", ")} onChange={e=>update("examInterests",e.target.value.split(",").map(v=>v.trim()).filter(Boolean))} placeholder="SBI PO, IBPS PO" /></label><label>Email<input value={profile.email||""} readOnly /></label><label>Authentication provider<input value={profile.provider||""} readOnly /></label><label>Account created<input value={formatDate(profile.createdAt)} readOnly /></label><label>Last updated<input value={formatDate(profile.updatedAt)} readOnly /></label></div><p className="privacy-note">Your mobile number is private and may be used for account, mentoring, support, and service-related communication. It is not verified until an OTP feature is completed, and marketing consent is not enabled automatically.</p><div className="sticky-form-actions"><button className="primary-button" type="submit" disabled={saving}>{saving?"Saving...":"Save profile"}</button></div>{status&&<p className="form-message" role="status">{status}</p>}</form>
}
function StudentNotificationSettings(){
 const [data,setData]=useState(null),[message,setMessage]=useState("Loading settings..."),[connecting,setConnecting]=useState(false),[disconnectOpen,setDisconnectOpen]=useState(false);
 async function load(){try{setData(await getNotificationPreferences());setMessage("")}catch(e){setMessage(e.message)}}useEffect(()=>{load();const refresh=()=>load();window.addEventListener("focus",refresh);return()=>window.removeEventListener("focus",refresh)},[]);
 async function save(){setMessage("Saving...");try{const r=await saveNotificationPreferences(data.preferences);setData({...data,preferences:r.preferences,telegramConnection:r.telegramConnection||data.telegramConnection});setMessage("Settings saved.")}catch(e){setMessage(e.message)}}
 async function connect(){if(connecting)return;const popup=window.open("","_blank");setConnecting(true);setMessage("Creating a secure Telegram connection link...");try{const result=await connectTelegramNotifications();if(popup){popup.opener=null;popup.location.href=result.link.url}else{window.location.href=result.link.url}setMessage("Telegram opened. Press Start in the bot, then return here to confirm the connection.");}catch(e){if(popup)popup.close();setMessage(e.message)}finally{setConnecting(false)}}
 async function disconnect(){setMessage("Disconnecting Telegram...");try{const result=await disconnectTelegramNotifications();setData({...data,telegramConnection:result.connection,preferences:{...data.preferences,telegram:false}});setMessage("Telegram disconnected. Historical deliveries were preserved.");setDisconnectOpen(false)}catch(e){setMessage(e.message)}}
 if(!data)return <AdminEmptyState title="Notification settings" text={message} onRetry={load}/>;const p=data.preferences,telegram=data.telegramConnection||{connected:false,status:"not_connected",bot:"@DelightGuidanceBot"};
 return <section className="notification-settings"><h2>Notification settings</h2><h3>Transactional communication</h3>{["inApp","transactionalEmail","paymentNotifications","subscriptionNotifications","expiryReminders"].map(k=><label key={k}><input type="checkbox" checked={p[k]!==false} disabled={k==="inApp"||(["transactionalEmail"].includes(k)&&!data.providers.email.enabled)} onChange={e=>setData({...data,preferences:{...p,[k]:e.target.checked}})}/>{titleLabel(k)} {k==="inApp"?"— Available":k==="transactionalEmail"&&!data.providers.email.enabled?"— Configuration required":""}</label>)}<section className="telegram-settings-card"><div><h3>Telegram</h3><p><strong>Status:</strong> {telegram.connected?"Connected":"Not connected"}</p><p><strong>Bot:</strong> {telegram.bot||"@DelightGuidanceBot"}</p>{telegram.chat&&<p><strong>Chat:</strong> {telegram.chat}</p>}</div>{telegram.connected?<><label><input type="checkbox" checked={p.telegram!==false} onChange={e=>setData({...data,preferences:{...p,telegram:e.target.checked}})}/>Telegram notifications enabled</label><button className="ghost-button" type="button" onClick={()=>setDisconnectOpen(true)}>Disconnect Telegram</button></>:<><p>Press Connect Telegram, then press Start in Telegram. The secure link expires after 10 minutes and can be used once.</p><button className="primary-button" type="button" disabled={connecting||!data.providers.telegram.enabled} onClick={connect}>{connecting?"Connecting...":"Connect Telegram"}</button>{!data.providers.telegram.enabled&&<p>Telegram configuration is required.</p>}</>}</section><h3>Optional reminders and marketing</h3>{["whatsapp","targetReminders","resourceNotifications","classNotifications","announcements","marketing"].map(k=><label key={k}><input type="checkbox" checked={Boolean(p[k])} disabled={k==="whatsapp"} onChange={e=>setData({...data,preferences:{...p,[k]:e.target.checked}})}/>{titleLabel(k)} {k==="whatsapp"?"— Configuration required":""}</label>)}<p>WhatsApp notifications require an official business provider and your consent.</p><button className="primary-button" type="button" onClick={save}>Save settings</button>{message&&<p className="form-message" role="status">{message}</p>}<ConfirmationDialog open={disconnectOpen} title="Disconnect Telegram?" message="Telegram delivery will stop. In-app and email notifications and historical delivery records will remain unchanged." onCancel={()=>setDisconnectOpen(false)} onConfirm={disconnect}/></section>
}
...
const analyticsColors=["#185adb","#d92d20","#0e9384","#7a5af8","#f79009","#039855","#e31b54","#667085"];
function AnalyticsLineChart({ analytics }){
 const definitions=analytics?.definitions||[],series=analytics?.series||{};const [selected,setSelected]=useState(()=>{try{return JSON.parse(localStorage.getItem("admin_analytics_series"))||["newUsers","successfulPayments","totalVerifiedRevenue"]}catch{return["newUsers","successfulPayments","totalVerifiedRevenue"]}});
 useEffect(()=>{localStorage.setItem("admin_analytics_series",JSON.stringify(selected))},[selected]);const chosen=definitions.filter(item=>selected.includes(item.key)),available=chosen.filter(item=>series[item.key]?.available),labels=[...new Set(available.flatMap(item=>(series[item.key].points||[]).map(point=>point.label)))],countMax=Math.max(1,...available.filter(x=>x.unit==="count").flatMap(x=>(series[x.key].points||[]).map(p=>Number(p.value)||0))),currencyMax=Math.max(1,...available.filter(x=>x.unit==="currency").flatMap(x=>(series[x.key].points||[]).map(p=>Number(p.value)||0)));function points(item){const values=new Map((series[item.key]?.points||[]).map(p=>[p.label,Number(p.value)||0])),max=item.unit==="currency"?currencyMax:countMax;return labels.map((label,index)=>`${labels.length<2?40:40+index*(620/(labels.length-1))},${220-(values.get(label)||0)/max*180}`).join(" ")}
 return <section className="admin-card analytics-panel"><header><div><p className="eyebrow">Trend analytics</p><h2>Selectable metric trends</h2><p>Counts use the left scale; INR uses the right scale. Stock metrics without historical snapshots are clearly marked unavailable.</p></div><div className="form-actions"><button className="ghost-button" type="button" onClick={()=>setSelected(definitions.map(x=>x.key))}>Select all</button><button className="ghost-button" type="button" onClick={()=>setSelected([])}>Clear all</button></div></header><div className="analytics-toggles">{definitions.map((item,index)=><label key={item.key}><input type="checkbox" checked={selected.includes(item.key)} onChange={()=>setSelected(current=>current.includes(item.key)?current.filter(key=>key!==item.key):[...current,item.key])}/><span style={{"--series-color":analyticsColors[index%analyticsColors.length]}} />{item.label}<small>{item.kind}</small></label>)}</div>{chosen.filter(item=>!series[item.key]?.available).map(item=><p className="analytics-unavailable" key={item.key}><strong>{item.label}:</strong> {series[item.key]?.reason||"Historical data unavailable."}</p>)}{available.length&&labels.length?<><div className="analytics-chart-scroll"><svg className="analytics-line-chart" viewBox="0 0 700 260" role="img" aria-label="Selected analytics trends"><line x1="40" y1="220" x2="660" y2="220" /><line x1="40" y1="40" x2="40" y2="220" />{available.map((item,index)=><polyline key={item.key} points={points(item)} fill="none" stroke={analyticsColors[definitions.findIndex(x=>x.key===item.key)%analyticsColors.length]} strokeWidth="3" tabIndex="0" aria-label={`${item.label} trend`}><title>{item.label}: focus this series and use the accessible table for exact values.</title></polyline>)}{labels.map((label,index)=><text key={label} x={labels.length<2?40:40+index*(620/(labels.length-1))} y="245" textAnchor="middle">{label}</text>)}</svg></div><div className="analytics-legend">{available.map(item=><span key={item.key}>{item.label} ({item.unit==="currency"?"INR":"count"})</span>)}</div><div className="admin-table-wrap analytics-table"><table className="admin-table"><thead><tr><th>Date</th>{available.map(item=><th key={item.key}>{item.label}</th>)}</tr></thead><tbody>{labels.map(label=><tr key={label}><td>{label}</td>{available.map(item=>{const value=(series[item.key].points||[]).find(point=>point.label===label)?.value||0;return <td key={item.key}>{item.unit==="currency"?formatPrice(value):value}</td>})}</tr>)}</tbody></table></div></>:<AdminEmptyState title="No historical series selected" text="Select an available flow metric to display its real recorded history." />}</section>
}
...
function AdminUsersPage({ admin }) {
  const [filters, setFilters] = useState({ page: 1, pageSize: 20, sort: "registered_desc" });
  const [data, setData] = useState(null);
  ...
  return <PermissionGate admin={admin} permission="users.view"><AdminPageHeader eyebrow="Users" title="Registered users" description="Firebase Authentication users merged with safe Firestore profile and subscription state." admin={admin} /><AdminOperationalFilters filters={filters} setFilters={setFilters} type="users" onExport={exportCsv} canExport={adminHasPermission(admin, "reports.export")} />{message ? <AdminEmptyState title="Unable to load users" text={message} onRetry={() => setFilters({ ...filters })} /> : data?.items?.length ? <><section className="admin-card admin-table-wrap"><table className="admin-table"><thead><tr><th>User</th><th>Email</th><th>Phone</th><th>Provider</th><th>Verified</th><th>Profile</th><th>Status</th><th>Registered (IST)</th><th>Last sign-in</th><th>Active plans</th><th>Current plan</th><th>Expiry</th><th>Action</th></tr></thead><tbody>{data.items.map((user) => <tr key={user.uid}><td>{user.photoURL ? <img className="admin-inline-avatar" src={user.photoURL} alt="" /> : <span className="admin-inline-avatar">{(user.displayName || "U").slice(0, 1)}</span>}{user.displayName}</td><td>{user.email}</td><td>{user.phone || "-"}</td><td>{user.provider}</td><td>{user.emailVerified ? "Verified" : "Unverified"}</td><td>{titleLabel(user.profileStatus)}</td><td><span className={`admin-status-badge ${user.accountStatus}`}>{user.accountStatus}</span></td><td>{formatRegistrationDate(user.createdAt)}</td><td>{formatDate(user.lastSignInAt)}</td><td>{user.activeSubscriptionCount}</td><td>{user.currentPlan}</td><td>{formatDate(user.subscriptionExpiry)}</td><td><a className="text-button" href={`${appBase}admin/users/${encodeURIComponent(user.uid)}`}>Open Profile</a></td></tr>)}</tbody></table><div className="admin-mobile-list">{data.items.map((user) => <article className="admin-mobile-card" key={user.uid}><header><div><strong>{user.displayName || user.email || "User"}</strong><span>{user.email || "No email"}</span></div><span className={`admin-status-badge ${user.accountStatus}`}>{user.accountStatus}</span></header><div className="admin-mobile-field"><span>Phone</span><strong>{user.phone || "-"}</strong></div><div className="admin-mobile-field"><span>Provider</span><strong>{user.provider || "-"}</strong></div><div className="admin-mobile-field"><span>Verified</span><strong>{user.emailVerified ? "Verified" : "Unverified"}</strong></div><div className="admin-mobile-field"><span>Profile</span><strong>{titleLabel(user.profileStatus)}</strong></div><div className="admin-mobile-field"><span>Registered (IST)</span><strong>{formatRegistrationDate(user.createdAt)}</strong></div><div className="admin-mobile-field"><span>Last sign-in</span><strong>{formatDate(user.lastSignInAt)}</strong></div><div className="admin-mobile-field"><span>Active plans</span><strong>{user.activeSubscriptionCount}</strong></div><div className="admin-mobile-field"><span>Current plan</span><strong>{user.currentPlan || "No active plan"}</strong></div><div className="admin-mobile-field"><span>Expiry</span><strong>{formatDate(user.subscriptionExpiry)}</strong></div><div className="admin-row-actions"><a className="ghost-button" href={`${appBase}admin/users/${encodeURIComponent(user.uid)}`}>Open Profile</a></div></article>)}</div></section><AdminPager {...data} onPage={(page) => setFilters({ ...filters, page })} /></> : <AdminEmptyState title="No users found" text="No registered users match the selected filters." />}</PermissionGate>;
}
...
function AdminTransactionsPage({ admin }) {
  const [filters, setFilters] = useState({ page: 1, pageSize: 20, sort: "transaction_desc" });
  const [data, setData] = useState(null);
...
function AdminNotificationsPage({ admin, id = "" }) {
  ...
  const [telegramWebhook, setTelegramWebhook] = useState({ busy: false, status: "", url: "" });
  async function manageWebhook(action) { if (telegramWebhook.busy) return; setTelegramWebhook((current) => ({ ...current, busy: true })); try { const result = await manageAdminTelegramWebhook(action); setTelegramWebhook({ busy: false, ...result }); } catch (error) { setTelegramWebhook({ busy: false, status: error.message || "Telegram API error", url: "" }); } }
